...
const FormField = ({ field, index, updateField, removeField, addOption, updateOption, removeOption, fields }: {
  field: Field;
  index: number;
  ...
  return (
    <Reorder.Item key={field.id} value={field} dragControls={controls} dragListener={false}>
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        exit={{ opacity: 0, x: 20 }}
        transition={{ duration: 0.3 }}
        className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow mb-4 flex"
      >
        <div className="w-8 bg-gray-200 rounded-l-lg flex items-center justify-center cursor-move" onPointerDown={(e) => controls.start(e)}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            <input
              ...
              className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#397698] focus:border-transparent duration-200 transition-all"
              required
            />
          </motion.div>
